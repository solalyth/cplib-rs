...
impl _O {
    pub fn init(&self, p: usize, max: usize) {
        let (mut inv, mut f, mut finv) = (vec![0; max+1], vec![0; max+1], vec![0; max+1]);
        // inv
        inv[1] = 1;
        for i in 2..=max { inv[i] = p - (p/i * inv[p%i] % p); }
    ...
    pub fn f(&self, n: usize) -> usize { unsafe { F[n] } }
    
    pub fn finv(&self, n: usize) -> usize { unsafe { FINV[n] } }
}
