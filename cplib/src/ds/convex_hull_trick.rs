...
fn f(l: (i64, i64), r: (i64, i64)) -> i64 {
    debug_assert!(l.0 < r.0);
    let res = (l.1 - r.1).div_euclid(r.0 - l.0);
    res
}
...
impl ConvexHullTrick {
    ...
    pub fn add_line(&mut self, a: i64, b: i64) {
        ...
        for &lnx in self.set.range(..=a).rev() {
            if let Some(l) = &mut l {
                ...
                *l = lnx;
                ...
                if lnx.r.0 < f(lnx.ab(), (a, b)) { return; }
                del.push(lnx.a);
                l = Some(lnx);
            }
        }
        ...
        for rnx in self.set.range(a+1..) {
            if f((a, b), rnx.ab()) < rnx.r.0 { qr = f((a, b), rnx.ab()); break; }
            del.push(rnx.a);
        }
